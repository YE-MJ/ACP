...
def lightgbm_ensemble():
    # 테스트할 CSV 파일 경로 설정
    test_file_path = './feature/csv/independent/'   # 여기에 테스트 파일 경로를 입력해주세요
    ...
    for test_file in test_files:
        ...
        for file_path in csv_files:
            ...
            # 모델 가중치 파일 경로 설정
            weight_file_paths = [
                f"./selected/best_model_lightgbm_{file_name.replace('.csv', '')}_fold_1.pkl",
                f"./selected/best_model_lightgbm_{file_name.replace('.csv', '')}_fold_2.pkl",
                f"./selected/best_model_lightgbm_{file_name.replace('.csv', '')}_fold_3.pkl",
                f"./selected/best_model_lightgbm_{file_name.replace('.csv', '')}_fold_4.pkl",
                f"./selected/best_model_lightgbm_{file_name.replace('.csv', '')}_fold_5.pkl"
            ]
