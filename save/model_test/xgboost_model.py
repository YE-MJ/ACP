...
def xgboost_ensemble():
    ...
    for test_file in test_files:
        ...
        for file_path in csv_files:
            ...
            # 모델 가중치 파일 경로 설정
            weight_file_paths = [
                f"./selected/best_model_xgboost_{file_name.replace('.csv', '')}_fold_1.pkl",
                f"./selected/best_model_xgboost_{file_name.replace('.csv', '')}_fold_2.pkl",
                f"./selected/best_model_xgboost_{file_name.replace('.csv', '')}_fold_3.pkl",
                f"./selected/best_model_xgboost_{file_name.replace('.csv', '')}_fold_4.pkl",
                f"./selected/best_model_xgboost_{file_name.replace('.csv', '')}_fold_5.pkl"
            ]
