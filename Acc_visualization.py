    ...
    with open(file_name, 'r') as f:
        ...
        for line in lines:
            if line.startswith("Dataset"):
                ...
                accuracy = float(re.search(r'Accuracy: (.+)', line).group(1))
                if dataset_name not in accuracy_dict:
                    accuracy_dict[dataset_name] = []
                accuracy_dict[dataset_name].append(accuracy)
...
for i in range(len(datasets)):
    for j in range(len(model_names)):
        accuracy_values[i, j] = accuracy_dict[datasets[i]][j]
