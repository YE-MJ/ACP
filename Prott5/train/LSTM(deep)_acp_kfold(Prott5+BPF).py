...
X_prott5 = embeddings_df.iloc[:, 1:-1].values  
...
X_combined = np.hstack((X_prott5, bpf_features_padded))

scaler = StandardScaler()
X_combined_scaled = scaler.fit_transform(X_combined)
joblib.dump(scaler, './Prott5/LSTM(deep)_Prott5+BPF_weights/scaler_LSTM(deep)_Prott5+BPF.pkl')
...
for fold, (train_index, test_index) in enumerate(kf.split(X_tensor), 1):
    ...
    for epoch in range(epochs):
        ...
        if loss.item() < best_loss:
            ...
            torch.save(best_model_wts, f'./Prott5/LSTM(deep)_Prott5+BPF_weights/best_model_fold_{fold}_loss.pth')
        
        if roc_auc > best_auc:
            best_auc = roc_auc
            best_model_wts = model.state_dict()
            torch.save(best_model_wts, f'./Prott5/LSTM(deep)_Prott5+BPF_weights/best_model_fold_{fold}_auc.pth')
    ...
    if acc > best_acc:
        best_acc = acc
        best_model_wts = model.state_dict()
        torch.save(best_model_wts, f'./Prott5/LSTM(deep)_Prott5+BPF_weights/best_model_fold_{fold}_acc.pth')
