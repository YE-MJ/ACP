...
import pandas as pd 
from utils import calculate_metrics, calculate_additional_metrics, plot_roc_curve, plot_confusion_matrix

# 데이터 import 
file_path = './Dataset/train/protein_embeddings_prott5.csv'
embeddings_df = pd.read_csv(file_path)

X = embeddings_df.iloc[:, 1:-1].values 
y = embeddings_df['target'].values

scaler = StandardScaler()
X_scaled = scaler.fit_transform(X)
joblib.dump(scaler, './Prott5/LSTM(deep)_Prott5_weights/scaler_LSTM(deep)_Prott5.pkl')
...
for fold, (train_index, test_index) in enumerate(kf.split(X_tensor), 1):
    ...
    for epoch in range(epochs):
        ...
        if loss.item() < best_loss:
            ...
            torch.save(best_model_wts, f'./Prott5/LSTM(deep)_Prott5_weights/best_model_fold_{fold}_loss.pth')
        
        if roc_auc > best_auc:
            best_auc = roc_auc
            best_model_wts = model.state_dict()
            torch.save(best_model_wts, f'./Prott5/LSTM(deep)_Prott5_weights/best_model_fold_{fold}_auc.pth')
    ...
    if acc > best_acc:
        best_acc = acc
        best_model_wts = model.state_dict()
        torch.save(best_model_wts, f'./Prott5/LSTM(deep)_Prott5_weights/best_model_fold_{fold}_acc.pth')
