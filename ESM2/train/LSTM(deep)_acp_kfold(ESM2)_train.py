...
joblib.dump(scaler, './ESM2/LSTM(deep)_ESM2_weights/scaler_LSTM(deep)_ESM2.pkl')
...
for fold, (train_index, test_index) in enumerate(kf.split(X_tensor), 1):
    ...
    for epoch in range(epochs):
        ...
        if loss.item() < best_loss:
            ...
            torch.save(best_model_wts, f'./ESM2/LSTM(deep)_ESM2_weights/best_model_fold_{fold}_loss.pth')
        
        if roc_auc > best_auc:
            best_auc = roc_auc
            best_model_wts = model.state_dict()
            torch.save(best_model_wts, f'./ESM2/LSTM(deep)_ESM2_weights/best_model_fold_{fold}_auc.pth')
    ...
    if acc > best_acc:
        best_acc = acc
        best_model_wts = model.state_dict()
        torch.save(best_model_wts, f'./ESM2/LSTM(deep)_ESM2_weights/best_model_fold_{fold}_acc.pth')
