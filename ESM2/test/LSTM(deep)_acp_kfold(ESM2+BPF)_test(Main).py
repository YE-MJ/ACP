...
# Main Test 
new_data_file_path = './Dataset/test/protein_embeddings_esm2.csv'
...
positive_file_path = './Dataset/main_ACP_data/antiCP2_test_ACP_data(positive).txt'
negative_file_path = './Dataset/main_ACP_data/antiCP2_test_ACP_data(negative).txt'
...
X_combined_new = np.hstack((X_esm2_new, bpf_features_padded_new))

scaler = joblib.load('./ESM2/LSTM(deep)_ESM2+BPF_weights/scaler_LSTM(deep)_ESM2+BPF.pkl')
...
best_model_paths_acc = [
    './ESM2/LSTM(deep)_ESM2+BPF_weights/best_model_fold_1_acc.pth',
    './ESM2/LSTM(deep)_ESM2+BPF_weights/best_model_fold_2_acc.pth',
    './ESM2/LSTM(deep)_ESM2+BPF_weights/best_model_fold_3_acc.pth',
    './ESM2/LSTM(deep)_ESM2+BPF_weights/best_model_fold_4_acc.pth',
    './ESM2/LSTM(deep)_ESM2+BPF_weights/best_model_fold_5_acc.pth'
]
...
result_df.to_csv('./ESM2/results/ESM2+BPF_test_results(Main)_acc.csv', index=False)
